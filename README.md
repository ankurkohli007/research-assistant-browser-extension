...
**Research Assistant Pro** is a full-stack AI-powered Chrome Extension that transforms how you research on the web. Highlight any text on any webpage, and instantly get an AI-generated summary, ask contextual follow-up questions, generate formatted academic citations, bookmark sections with deep-link anchors, and export your entire research workspace — all without leaving your browser.

The backend is a reactive REST API built with **Spring Boot + WebFlux**, integrating **Google Gemini AI** for natural language understanding. The frontend is a polished Chrome Side Panel Extension built in vanilla JavaScript with a premium SaaS-grade UI.
...
| Layer | Technology |
|---|---|
| **Frontend** | Vanilla JavaScript (ES2022), Chrome Extensions Manifest V3, Chrome Side Panel API |
| **Styling** | Custom CSS Design System with CSS Variables, responsive layout |
| **Backend** | Java 17, Spring Boot 3, Spring WebFlux (Reactive) |
...
Chrome Extensions have strict CSP rules. Vanilla JS avoids build tooling complexity, keeps the extension lightweight, and demonstrates deep DOM/browser API knowledge without framework abstractions.

**Why WebFlux over Spring MVC?**
Gemini API calls are I/O-bound and can take 1–3 seconds. WebFlux handles these with non-blocking `Mono` pipelines, keeping the server thread-efficient — important if this scales to multiple users.
